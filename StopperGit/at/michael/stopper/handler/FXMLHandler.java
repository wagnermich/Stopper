...
public abstract class FXMLHandler extends StackPane implements IView{
    ...
    */
   public FXMLHandler(Function<String, Object> injectionContext) {
   ...
   FXMLLoader loadSynchronously(final URL resource, ResourceBundle bundle, final String conventionalName) throws IllegalStateException {
	   ...
	   IPresenterFactory factory = discover();
   ...
   IPresenterFactory discover() {
       Iterable<IPresenterFactory> discoveredFactories = IPresenterFactory.discover();
       List<IPresenterFactory> factories = StreamSupport.stream(discoveredFactories.spliterator(), false).
               collect(Collectors.toList());
       if (factories.isEmpty()) {
           return InjectionHandler::instantiatePresenter;
       }
   ...
   //getView() liefert immer den Root
   public Node getRootContainer(Node node) {
	   if(node.getParent()!=null)
	     node=getRootContainer(node.getParent());
       return node;
   }
   ...
   public void verankern(Node node, double padding) {
   ...
   public void verankern(Node node) {
	   	verankern(node, 0.0);
  	
  }
  public Node add(Node node) {
	  if(this.getView() instanceof ScrollPane) {
